...
function App() {
  ...
  // Protección de rutas - redirigir a login si no hay acceso
  useEffect(() => {
    if (!access && pathname !== "/" && pathname !== "/register") {
      navigate("/");
    }
  ...
  const fetchAllCharacters = async (page) => {
    try {
      ...
      // Normalizar origin a string para cada personaje
      const normalizedCharacters = response.results.map((char) => ({
        ...char,
